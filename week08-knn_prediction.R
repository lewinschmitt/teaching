...
pacman::p_load(tidyverse, caret, class)
...
data <- read.csv('https://www.chesdata.eu/s/2014_CHES_dataset_means.csv')
...
# Coding rule: anti = 0, pro = 1
median(data$eu_position)
data <- data %>%
  mutate(eu_stance = ifelse(eu_position < 5.5, 'anti','pro'))
table(data$eu_stance) # our outcome classes are nicely balanced (because we designed it this way)
...
knn_data <- data %>%
  select(eu_stance, urban_rural, immigrate_policy, environment) %>%
  na.omit()
predictors <- knn_data[,c('urban_rural','immigrate_policy')]
outcomes <- factor(knn_data[,'eu_stance'])

is.na(predictors)
eu_predictions_knn <- class::knn(train = predictors, 
                          test = predictors,
                   cl = outcomes,
                   k = 1)
...
# So let's increase k to decrease the model flexibility:

eu_predictions_knn_20 <- class::knn(train = predictors, 
                          test = predictors,
                          cl = outcomes, 
                          k = 20)
...
caret::confusionMatrix(eu_predictions_knn_20,factor(outcomes))
...
for(k in 1:nrow(knn_data)){
  eu_prediction.temp<-class::knn(train = predictors, test = predictors,
                          cl = outcomes, k = k)
  results$accuracy[k] <-
...
for(k in 1:(nrow(knn_data)-1)){
  for(lo in 1:nrow(knn_data)){
    ...
    predict_test_loocv <- class::knn(train = train.predictors.loocv,test = test.predictors.loocv,
...
# Use the best model you can train on the 2014 survey, and see how well it does on predicting EU stance with the more recent 2019 data.
# Send me your (commented!) code ahead of next week's class. 
